...
class DefaultFixtureImporterTest extends WebTestCase
{
    ...
    public function testLoadAlbumsFromJSONGood()
    {
...
JSON_FRAGMENT;
        ...
        $albums = $fixtureImporter->loadAlbumsFromJSON($testJSONFragment);

        $this->assertEquals(2, count($albums));
        $this->assertEquals('Album 1', $albums[0]->getName());
    }

    public function testLoadAlbumsFromJSONBad()
    {
        // Initialize SUT object $fixtureImporter
        ...
        $this->assertEmpty($fixtureImporter->loadAlbumsFromJSON(''));
    }
